from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
NATIVE = ROOT / "build" / "linux" / "project"
if NATIVE.is_dir() and str(NATIVE) not in sys.path:
    sys.path.insert(0, str(NATIVE))

from experiments.stride_v2first_consensus16_warehouse import (  # noqa: E402
    analyze_final,
    analyze_stage_a,
    plan,
    run,
    run_stage_a,
    run_stage_b,
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the staged fresh-seed w1020a V2-first consensus16 rescue screen."
        )
    )
    parser.add_argument(
        "command",
        choices=(
            "plan",
            "run",
            "run-stage-a",
            "analyze-stage-a",
            "run-stage-b",
            "analyze-final",
        ),
    )
    parser.add_argument("--config", required=True)
    parser.add_argument("--output")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    config = Path(args.config)
    if args.command == "plan":
        if args.output or args.resume or args.dry_run:
            parser.error("plan accepts only --config")
        result = plan(config)
    else:
        if not args.output:
            parser.error(f"--output is required for {args.command}")
        output = Path(args.output)
        if args.command in {"analyze-stage-a", "analyze-final"}:
            if args.resume or args.dry_run:
                parser.error("analysis commands do not accept --resume/--dry-run")
            result = (
                analyze_stage_a(config, output)
                if args.command == "analyze-stage-a"
                else analyze_final(config, output)
            )
        elif args.command == "run-stage-a":
            result = run_stage_a(
                config, output, resume=args.resume, dry_run=args.dry_run
            )
        elif args.command == "run-stage-b":
            result = run_stage_b(
                config, output, resume=args.resume, dry_run=args.dry_run
            )
        else:
            result = run(
                config, output, resume=args.resume, dry_run=args.dry_run
            )
    print(json.dumps(result, indent=2, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
