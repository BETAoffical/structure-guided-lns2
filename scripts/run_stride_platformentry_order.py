...
from experiments.stride_platformentry_order import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the preregistered bounded first-action PP-order platform mechanism."
        )
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--config",
        default="configs/stride_platformentry_order_v1_registration.json",
    )
    parser.add_argument(
        "--output",
        default="build/stride-platformentry-order-v1",
    )
    ...
    if arguments.command == "collect-initial":
        ...
        report = analyze_order_collection(
            arguments.config,
            arguments.output,
            phase="initial",
            expected_cases=arguments.limit_cases or 36,
        )
    else:
        report = analyze_order_collection(
            arguments.config,
