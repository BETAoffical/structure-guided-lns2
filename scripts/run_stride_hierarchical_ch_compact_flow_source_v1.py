...
from experiments.stride_hierarchical_ch_compact_flow_source_v1 import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the registered compact-flow source cohort. Production collection "
            "qualifies both splits before any realized_dynamic episode."
        )
    )
    parser.add_argument(
        "--config",
        default="configs/stride_hierarchical_ch_compact_flow_source_v1.json",
    )
    parser.add_argument(
        "--output",
        default="build/stride-hierarchical-ch-compact-flow-source-v1",
    )
