...
from experiments.stride_causaltopopool_opportunity import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CausalTopoPool opportunity audit")
    parser.add_argument("command", choices=("preflight", "collect", "analyze"))
    parser.add_argument(
        "--execution",
        default=str(
            ROOT / "configs" / "stride_causaltopopool_v1_opportunity_execution.json"
        ),
    )
    ...
    if arguments.command == "preflight":
        result = collect_causaltopology_trials(
        ...
        result = analyze_causaltopology_trials(
            execution_path=arguments.execution,
