...
from experiments.stride_hierarchical_ch_source_v2 import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize and collect the registered bucket-mix source-v2 cohort. "
            "Production collection always qualifies train and development before policy execution."
        )
    )
    parser.add_argument(
        "--config",
        default="configs/stride_hierarchical_ch_source_v2.json",
    )
    parser.add_argument(
        "--output",
        default="build/stride-hierarchical-ch-source-v2",
    )
