...
from experiments.receding_q_variance_audit import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Decompose candidate, paired PP-seed, and interaction variance "
            "in existing four-seed receding-Q rollouts."
        )
    )
    parser.add_argument("--stability", required=True)
    parser.add_argument("--output", required=True)
    arguments = parser.parse_args()
    report = audit_receding_q_variance(
