...
`configs/movingai_mechanism_probe_dataset.json` selects two densities on each of:
...
```bash
python3 scripts/prepare_movingai_probe.py \
...
PYTHONPATH=build/linux/project python3 scripts/collect_repair_experience.py \
  ...
  --config configs/movingai_mechanism_probe_collection.json \
...
python3 scripts/analyze_movingai_probe.py \
