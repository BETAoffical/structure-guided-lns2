...
LNS::LNS(const Instance& instance, double time_limit, const string & init_algo_name, const string & replan_algo_name,
         ...
         bool repair_only, NeighborhoodPolicy* repair_policy, RepairObserver* repair_observer,
         ...
         path_table(instance.map_size), pipp_option(pipp_option), repair_only(repair_only),
         repair_policy(repair_policy), repair_observer(repair_observer),
...
bool LNS::run()
{
    ...
    if (!succ && initial_solution_runtime < time_limit)
    {
        if (use_init_lns)
        {
            init_lns = new InitLNS(instance, agents, time_limit - initial_solution_runtime,
                    ...
                    repair_policy, repair_observer, max_repair_iterations);
            succ = init_lns->run();
    ...
    while (!repair_only && runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
