...
The active runtime is intentionally compact: `src/`, `include/`, `experiments/`,
`scripts/`, `configs/`, and `generators/`. Historical studies and non-default
engineering prototypes live under `research/`, while their tests remain grouped
under `tests/research/`. Frozen deployable controllers are the only model files
kept in tracked `artifacts/`.
...
The 7,344-outcome context audit is implemented by `research/scripts/context/run_context_audit.py`. The current Pilot v2
...
are intentionally paused. See [`research/docs/context/CONTEXT_AUDIT.md`](research/docs/context/CONTEXT_AUDIT.md).

`research/scripts/representation/run_local_representation_audit.py` diagnoses whether that failure came from missing local
...
[`research/docs/representation/LOCAL_REPRESENTATION_AUDIT.md`](research/docs/representation/LOCAL_REPRESENTATION_AUDIT.md).
...
`research/scripts/neighborhood/prepare_movingai_probe.py` adapts the pinned standard maps into a fingerprinted probe split,
and `research/scripts/neighborhood/analyze_movingai_probe.py` separates immediate action effects from dual-trial noise and
tests map/density oracle heterogeneity at the task-instance level. See
[`research/docs/neighborhood/MOVINGAI_MECHANISM_PROBE.md`](research/docs/neighborhood/MOVINGAI_MECHANISM_PROBE.md).
...
`research/scripts/neighborhood/audit_movingai_probe_quality.py` performs the follow-up effective-sample and label-stability
...
[`research/docs/neighborhood/MOVINGAI_PROBE_QUALITY.md`](research/docs/neighborhood/MOVINGAI_PROBE_QUALITY.md).
...
`research/scripts/neighborhood/analyze_independent_layout_probe.py` implements that bounded confirmation without training a
...
[`research/docs/neighborhood/INDEPENDENT_LAYOUT_PROBE.md`](research/docs/neighborhood/INDEPENDENT_LAYOUT_PROBE.md).
...
The next bounded experiment is implemented by `research/scripts/neighborhood/collect_realized_neighborhood_probe.py`. It
...
[`research/docs/neighborhood/REALIZED_NEIGHBORHOOD_PROBE.md`](research/docs/neighborhood/REALIZED_NEIGHBORHOOD_PROBE.md).
...
That audit is now implemented by `research/scripts/neighborhood/run_realized_neighborhood_ranking_audit.py`. It aggregates
...
[`research/docs/neighborhood/REALIZED_NEIGHBORHOOD_RANKING_AUDIT.md`](research/docs/neighborhood/REALIZED_NEIGHBORHOOD_RANKING_AUDIT.md).

The next registered stage is implemented by `research/scripts/neighborhood/collect_realized_ranking_confirmation.py` and
`research/scripts/neighborhood/run_realized_ranking_confirmation.py`. A proposal-only native API generates candidate agent
...
[`research/docs/neighborhood/REALIZED_RANKING_CONFIRMATION.md`](research/docs/neighborhood/REALIZED_RANKING_CONFIRMATION.md).
...
[`research/docs/neighborhood/NATURAL_DISTRIBUTION_CONFIRMATION.md`](research/docs/neighborhood/NATURAL_DISTRIBUTION_CONFIRMATION.md).
...
[`research/docs/sequential/SEQUENTIAL_CREDIT_AUDIT.md`](research/docs/sequential/SEQUENTIAL_CREDIT_AUDIT.md).
...
[`research/docs/sequential/REPAIR_ORDER_PROBE.md`](research/docs/sequential/REPAIR_ORDER_PROBE.md).
...
[`research/docs/context/CONTEXTUAL_REPAIR_ORDER_AUDIT.md`](research/docs/context/CONTEXTUAL_REPAIR_ORDER_AUDIT.md).
...
- [`research/docs/context/CONTEXT_AUDIT.md`](research/docs/context/CONTEXT_AUDIT.md): pairwise ablations, gates, and current negative result.
- [`research/docs/context/CONTEXT_SECONDARY_AUDIT.md`](research/docs/context/CONTEXT_SECONDARY_AUDIT.md): map-grouped diagnostics,
  context permutations, stopping rule, and the secondary negative result.
- [`research/docs/representation/LOCAL_REPRESENTATION_AUDIT.md`](research/docs/representation/LOCAL_REPRESENTATION_AUDIT.md): local path features,
...
- [`research/docs/neighborhood/INDEPENDENT_LAYOUT_PROBE.md`](research/docs/neighborhood/INDEPENDENT_LAYOUT_PROBE.md): independent layout/OD/density
  confirmation, staged collection gates, and registered interpretation.
- [`research/docs/neighborhood/REALIZED_NEIGHBORHOOD_PROBE.md`](research/docs/neighborhood/REALIZED_NEIGHBORHOOD_PROBE.md): explicit-neighborhood
...
- [`research/docs/representation/GRAPH_REPRESENTATION_AUDIT.md`](research/docs/representation/GRAPH_REPRESENTATION_AUDIT.md): neural flat, agent-set,
  and conflict-GNN comparison against the registered GBDT.
- [`research/docs/representation/GRAPH_FEATURE_GBDT_AUDIT.md`](research/docs/representation/GRAPH_FEATURE_GBDT_AUDIT.md): deterministic structural and
  temporal conflict-graph feature bridge and its final negative Train result.
- [`research/docs/sequential/SEQUENTIAL_CREDIT_AUDIT.md`](research/docs/sequential/SEQUENTIAL_CREDIT_AUDIT.md): Train-only frozen-policy
  Horizon-4 collection, split-trial stability, and sequential-credit gates before any RL work.
- [`research/docs/sequential/REPAIR_ORDER_PROBE.md`](research/docs/sequential/REPAIR_ORDER_PROBE.md): explicit PP order control, common-random-number
  trials, and the mechanism gate for redefining the InitLNS action.
- [`research/docs/context/CONTEXTUAL_REPAIR_ORDER_AUDIT.md`](research/docs/context/CONTEXTUAL_REPAIR_ORDER_AUDIT.md): Train-only contextual
...
- [`research/`](research/README.md): historical studies, negative results, and non-default
  engineering prototypes, organized separately from the active controller.
- The deleted simplified solver and Stage 3-5 code remain recoverable from the
