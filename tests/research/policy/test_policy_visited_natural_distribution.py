...
from research.studies.policy.policy_visited_independent_confirmation import _validate_config
...
class PolicyVisitedNaturalDistributionTests(unittest.TestCase):
    ...
    def test_confirmation_configs_are_fixed_and_separate(self) -> None:
        config = json.loads(
            (PROJECT_ROOT / "research/configs/policy/policy_visited_independent_confirmation.json").read_text(
                encoding="utf-8"
            )
        )
        _validate_config(config)
        dataset = json.loads(
            (PROJECT_ROOT / "research/configs/policy/policy_visited_confirmation_dataset.json").read_text(
                encoding="utf-8"
            )
        )
