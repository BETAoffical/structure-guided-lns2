...
class SequentialCreditAuditTests(unittest.TestCase):
    ...
    def test_early_feasible_trial_pads_horizon_with_zero(self) -> None:
        ...
        with patch("research.studies.sequential.sequential_credit_audit.analyze_static_grid", return_value=None):
    ...
    def test_frozen_continuation_runs_after_initial_candidate(self) -> None:
        ...
        with (
            patch("research.studies.sequential.sequential_credit_audit.analyze_static_grid", return_value=None),
            patch(
                "research.studies.sequential.sequential_credit_audit.generate_online_candidates",
                return_value=([continuation], {"proposal_count": 1, "candidate_count": 1, "proposal_seconds": 0.0}),
            ),
            patch("research.studies.sequential.sequential_credit_audit.online_candidate_rows", return_value=[{"features": {"realized_dynamic": {}}, "candidate_key": "continuation"}]),
            patch("research.studies.sequential.sequential_credit_audit.score_online_candidates", return_value=(0, [1.0], 1.0)),
            patch("research.studies.sequential.sequential_credit_audit.feature_range_diagnostic", return_value={"outside_fraction": 0.0}),
            patch("research.studies.sequential.sequential_credit_audit.repair_random_seed", return_value=123),
        ):
