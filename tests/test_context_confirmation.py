...
from experiments.context_confirmation import (
...
class ContextConfirmationTests(unittest.TestCase):
    ...
    def test_confirmation_configs_match_registered_volume(self) -> None:
        dataset = load_json(
            PROJECT_ROOT / "configs" / "context_confirmation_dataset.json"
        )
        collection = load_json(
            PROJECT_ROOT / "configs" / "context_confirmation_collection.json"
        )
