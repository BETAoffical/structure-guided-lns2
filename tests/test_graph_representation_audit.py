...
from experiments.graph_representation_audit import (
    MODEL_NAMES,
    NODE_FEATURE_NAMES,
    Normalization,
    ProgressLog,
...
class GraphRepresentationAuditTests(unittest.TestCase):
    def setUp(self) -> None:
        self.config = json.loads(
            (PROJECT_ROOT / "configs/graph_representation_audit.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_mismatched_output_fingerprint_is_rejected_before_indexing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "different run fingerprint"):
                run_graph_representation_audit(
                    PROJECT_ROOT,
                    PROJECT_ROOT / "configs/graph_representation_audit.json",
                    output,
                    phase="index",
                )
