...
class ClosedLoopConfirmationTests(unittest.TestCase):
    ...
    def test_retired_action_replacement_overrides_are_not_executable(self) -> None:
        for controller_mode in ("official_adaptive", "v2-full", "v3-s3"):
            for name in (
                ...
                with self.subTest(
                    controller_mode=controller_mode, name=name
    ...
    def test_v3_s3_rejects_forced_first_action(self) -> None:
        for name in (
            "forced_first_action",
        ):
    ...
    def test_dual16_realized_rows_use_one_final_union_batch(self) -> None:
        feature_values = {"candidate-a": 1.0, "candidate-m": 9.0, "candidate-z": 2.0}
        realized_calls: list[list[str]] = []
        scored_rows: list[list[dict]] = []
        generated_base_ids: list[str] = []
                ...
                return [
                    {
                        ...
                        "features": {
                            "realized_dynamic": {
                                "state.agent_count": feature_values[candidate_id]
                            }
                        },
                    }
        ...
        class FeatureScoreModel:
            ...
            def score_candidates(self, rows):
                scored_rows.append(json.loads(json.dumps(list(rows))))
                return [
                    float(row["features"]["realized_dynamic"]["state.agent_count"])
                    for row in rows
                ]
        ...
        challenger = make_candidate(
            "candidate-m", [0, 2], "structpool:spatiotemporal_hotspot:16"
        )
        ...
        def generated_dual16(
            _state, _analysis, *, v2_candidates, config
        ) -> HybridStructPoolResult:
            del config
            generated_base_ids.extend(
                str(candidate["candidate_id"]) for candidate in v2_candidates
            )
            by_id = {
                str(candidate["candidate_id"]): dict(candidate)
                for candidate in v2_candidates
            }
            union = [by_id["candidate-a"], dict(challenger), by_id["candidate-z"]]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            job = {
                ...
                "solver_seed": 0,
                "output_root": directory,
                "run_fingerprint": "dual16-single-union-batch",
                "resume": False,
                "dataset_root": directory,
                "environment": {"replan_algorithm": "PP"},
                "max_decisions": 1,
                ...
                "proposal": {
                    ...
                    "trials": 8,
                    "candidates_per_family": 2,
                    "hybridstructpool": structshell_dual16_augmentation(),
                },
                "frozen_models": directory,
                "model_registration": {},
                "controller": "v2-full",
        ...
        final_ids = ["candidate-a", "candidate-m", "candidate-z"]
        self.assertEqual(generated_base_ids, ["candidate-a", "candidate-z"])
        self.assertEqual(realized_calls, [final_ids])
        self.assertEqual(len(scored_rows), 1)
        self.assertEqual(
            [row["candidate_id"] for row in scored_rows[0]], final_ids
        )
        self.assertEqual(
            [
                row["features"]["realized_dynamic"]["state.agent_count"]
                for row in scored_rows[0]
            ],
            [1.0, 9.0, 2.0],
        )
        ...
        self.assertEqual(
            [candidate["candidate_id"] for candidate in controller["candidate_pool"]],
            final_ids,
        )
        self.assertEqual(controller["selected_candidate_id"], "candidate-m")
        proposal = controller["proposal"]
        self.assertTrue(proposal["hybridstructpool_single_union_feature_batch"])
        self.assertEqual(proposal["hybridstructpool_computed_union_feature_count"], 3)
        self.assertEqual(proposal["hybridstructpool_reused_base_feature_count"], 0)
        self.assertEqual(
    ...
    def test_structpool_gate_is_current_state_only_and_exact(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "unsupported StructPool"):
            validate_structpool_augmentation(retired_slotpool)

        retired_guardpool = json.loads(json.dumps(STRUCTPOOL_RUNTIME))
        retired_guardpool["stall_guard"] = {
            "no_progress_limit": 8,
            "wall_time_condition": None,
        }
        with self.assertRaisesRegex(ValueError, "unsupported StructPool"):
            validate_structpool_augmentation(retired_guardpool)
