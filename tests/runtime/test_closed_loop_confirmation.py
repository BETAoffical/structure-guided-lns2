...
class Dual16PlateauEnvironment:
    """Ten-decision plateau fixture with one guarded V2 progress step."""
    ...
    def step(self, action: dict) -> dict:
        ...
        agents = list(map(int, action["agents"]))
        before_conflicts = int(self.state["num_of_colliding_pairs"])
        if before_conflicts == 2 and agents == [0, 1]:
            after_conflicts = 1
        elif before_conflicts == 1 and agents != [0, 1]:
            after_conflicts = 0
        else:
            after_conflicts = before_conflicts
        solved = after_conflicts == 0
        conflict_edges = (
            [[0, 1], [2, 3]]
            if after_conflicts == 2
            else [[0, 1]]
            if after_conflicts == 1
            else []
        )
        self.state = {
            **self.state,
            ...
            "feasible": solved,
            "done": solved,
            "num_of_colliding_pairs": after_conflicts,
...
class ClosedLoopConfirmationTests(unittest.TestCase):
    ...
    def test_dual16_plateau_guard_closes_at_limit_and_releases_after_progress(
        self,
    ) -> None:
        environment = Dual16PlateauEnvironment()
        candidates = RollbackCandidateFixture()
        model = RollbackDirectCandidateModel()

        def gate_for_state(state, _config):
            edges = list(state["conflict_edges"])
            active = {int(agent) for edge in edges for agent in edge}
            return {
                "passed": True,
                "reason": "dual16_ablation_all_states",
                "seconds": 0.0,
                "gate_id": "dual16_ablation_all_states",
                "agent_count": len(state["agents"]),
                "conflict_pair_count": int(state["num_of_colliding_pairs"]),
                "active_conflict_agent_count": len(active),
                "largest_conflict_component_size": len(active),
            }

        with tempfile.TemporaryDirectory() as directory:
            job = rollback_worker_job(directory, max_decisions=10)
            job["run_fingerprint"] = "dual16-plateau-guard"
            job["proposal"]["hybridstructpool"] = (
                structshell_dual16_plateau_augmentation()
            )
            RollbackFeatureEngine.instances.clear()
        ...
        self.assertEqual(environment.step_calls, len(transitions))
        self.assertEqual(candidates.generation_calls, 10)
        self.assertEqual(gate.call_count, 9)
        self.assertEqual(generator.call_count, 9)

        proposals = [
            transition["controller"]["proposal"] for transition in transitions
        ]
        self.assertEqual(
            proposals[0]["hybridstructpool_selected_candidate_id"], "struct-b"
        )
        self.assertEqual(
            proposals[0]["hybridstructpool_stall_guard_no_progress_streak"], 0
        )
        self.assertFalse(proposals[0]["hybridstructpool_stall_guard_active"])
        self.assertEqual(
            proposals[7]["hybridstructpool_stall_guard_no_progress_streak"], 7
        )
        self.assertFalse(proposals[7]["hybridstructpool_stall_guard_active"])

        triggered = proposals[8]
        self.assertEqual(
            triggered["hybridstructpool_stall_guard_no_progress_streak"], 8
        )
        self.assertTrue(triggered["hybridstructpool_stall_guard_active"])
        self.assertTrue(triggered["hybridstructpool_stall_guard_triggered"])
        self.assertFalse(triggered["hybridstructpool_stall_guard_released"])
        self.assertFalse(triggered["hybridstructpool_gate_evaluated"])
        self.assertFalse(triggered["hybridstructpool_gate_passed"])
        self.assertEqual(
            triggered["hybridstructpool_gate_reason"], "plateau_guard_active"
        )
        self.assertEqual(
            triggered["hybridstructpool_selected_candidate_id"], "v2-anchor"
        )
        self.assertEqual(triggered["hybridstructpool_selected_provenance"], [])
        self.assertEqual(
            triggered.get("hybridstructpool_structural_candidate_count", 0), 0
        )

        released = proposals[9]
        self.assertEqual(
            released["hybridstructpool_stall_guard_no_progress_streak"], 0
        )
        self.assertFalse(released["hybridstructpool_stall_guard_active"])
        self.assertFalse(released["hybridstructpool_stall_guard_triggered"])
        self.assertTrue(released["hybridstructpool_stall_guard_released"])
        self.assertTrue(released["hybridstructpool_gate_evaluated"])
        self.assertTrue(released["hybridstructpool_gate_passed"])
        self.assertEqual(
            released["hybridstructpool_selected_candidate_id"], "struct-b"
        )

        totals = result["summary"]["controller_totals"]
        self.assertEqual(
            totals["hybridstructpool_stall_guard_active_decision_count"], 1
        )
        self.assertEqual(totals["hybridstructpool_stall_guard_trigger_count"], 1)
        self.assertEqual(totals["hybridstructpool_stall_guard_release_count"], 1)
        self.assertTrue(
            all(not row["metrics"]["pp_rolled_back"] for row in transitions)
        )
    ...
    def test_structpool_gate_is_current_state_only_and_exact(self) -> None:
        state = make_state()
        self.assertFalse(structpool_high_stress_gate(state, STRUCTPOOL_RUNTIME)["passed"])
        active = {
            **state,
        ...
        summary = structpool_high_stress_gate(active, STRUCTPOOL_RUNTIME)
        self.assertTrue(summary["passed"])
        self.assertEqual(summary["active_conflict_agent_count"], 32)
        self.assertEqual(summary["largest_conflict_component_size"], 2)
        changed = json.loads(json.dumps(STRUCTPOOL_RUNTIME))
        changed["maximum_added_candidates"] = 5
    ...
    def test_structpool_inactive_state_is_exact_base_fallback(self) -> None:
        ...
        with patch(
            ...
            side_effect=AssertionError("inactive gate must not analyze topology"),
        ):
            gate = structpool_high_stress_gate(state, STRUCTPOOL_RUNTIME)
            actual, metrics = generate_online_candidates(
                ...
                solver_seed=0,
                decision_index=0,
                proposal_config={**base_config, "structpool": STRUCTPOOL_RUNTIME},
                structpool_gate_result=gate,
            )
        self.assertEqual(actual, baseline)
        self.assertTrue(metrics["structpool_gate_evaluated"])
        self.assertFalse(metrics["structpool_gate_passed"])
        self.assertEqual(metrics["structpool_added_candidate_count"], 0)
        self.assertTrue(metrics["structpool_gate_precomputed"])

    def test_structpool_active_state_appends_capped_explicit_candidates(self) -> None:
        state = {
            **make_state(),
            "num_of_colliding_pairs": 16,
            "conflict_edges": [[2 * index, 2 * index + 1] for index in range(16)],
        ...
        with patch(
            "lns2_selector.runtime.online_selection.generate_structpool_candidates",
        ...
        self.assertTrue(metrics["structpool_gate_passed"])
    ...
    def test_structpool_lean_filter_removes_only_pure_bottleneck_before_merge(
        ...
        pure = {
            "candidate_id": "structpool-pure-bottleneck",
            "agents": [0, 3],
            "actual_size": 2,
            "selection_families": ["structpool-bottleneck-crossing:8"],
            "selection_rank_by_family": {"structpool-bottleneck-crossing:8": 0},
            "proposal_count_by_family": {"structpool-bottleneck-crossing:8": 1},
            "proposal_seeds": [],
            "seed_agents": [],
            "structpool_family_groups": ["bottleneck_crossing"],
            "structpool_score": 2.0,
        }
        mixed = {
            **pure,
            "candidate_id": "structpool-mixed-bottleneck",
            "agents": [1, 3],
            "structpool_family_groups": ["bottleneck_crossing", "path_overlap"],
            "structpool_score": 1.0,
        }
        retained, removed = filter_structpool_lean_candidates(
            [pure, mixed], STRUCTPOOL_LEAN_RUNTIME
        )
        self.assertEqual(retained, [mixed])
        self.assertEqual(removed, ["structpool-pure-bottleneck"])
        self.assertEqual(
            filter_structpool_lean_candidates([pure, mixed], STRUCTPOOL_RUNTIME),
            ([pure, mixed], []),
        )
        ...
        with patch(
            ...
            candidates, metrics = generate_online_candidates(
                ...
                proposal_config={
                    ...
                    "trials": 2,
                    "candidates_per_family": 1,
                    "structpool": STRUCTPOOL_LEAN_RUNTIME,
                },
                state_hash=state_fingerprint(state),
                verify_full_state=False,
                topology_static_grid=analyze_static_grid(state),
        ...
        self.assertTrue(metrics["structpool_lean_filter_enabled"])
        self.assertEqual(metrics["structpool_generated_count"], 2)
        self.assertEqual(metrics["structpool_retained_candidate_count"], 1)
        self.assertEqual(metrics["structpool_filtered_candidate_count"], 1)
        self.assertEqual(
            metrics["structpool_filtered_candidate_ids"],
            ["structpool-pure-bottleneck"],
        )
        self.assertNotIn("structpool-pure-bottleneck", {row["candidate_id"] for row in candidates})
        self.assertIn("structpool-mixed-bottleneck", {row["candidate_id"] for row in candidates})

        changed = json.loads(json.dumps(STRUCTPOOL_LEAN_RUNTIME))
        changed["lean_filter"]["preserve_mixed_family_candidates"] = False
        with self.assertRaisesRegex(ValueError, "unsupported StructPool"):
            validate_structpool_augmentation(changed)
